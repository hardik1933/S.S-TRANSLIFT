...
export function mapWorkerFromDb(row: WorkerRow) {
...
export function mapTransportRequestFromDb(row: Record<string, unknown>) {
...
export function mapTransportRequestFormToDb(
  ...
  if (!pickupDate) throw new Error('Service date is required');
